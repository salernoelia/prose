...
<template>
    ...
    <div
        ...
        :class="visible ? 'translate-y-0 opacity-100' : 'translate-y-16 opacity-0 pointer-events-none'"
    >
        <!-- Small Border Card (Rounded Pill) -->
        <div
            class="bg-(--bg-card) border border-(--border-color) rounded-full px-4 py-2 shadow-md flex items-center gap-4">
            <!-- Back to Library -->
            <button
                @click="emit('back')"
                class="flex items-center justify-center w-8 h-8 rounded-full text-(--text-secondary) hover:text-(--text-primary) transition-colors focus-ring-minimal"
            ...
            <button
                @click="emit('toc')"
                :disabled="!hasToc"
                class="flex items-center justify-center w-8 h-8 rounded-full text-(--text-secondary) hover:text-(--text-primary) transition-colors focus-ring-minimal disabled:opacity-20 disabled:hover:text-(--text-secondary)"
            ...
            <template v-if="canZoom">
                ...
                <div class="flex items-center gap-1 text-(--text-secondary) select-none">
                    <button
                        @click="emit('zoom-out')"
                        class="flex items-center justify-center w-6 h-6 rounded-full hover:text-(--text-primary) focus-ring-minimal"
                        title="Zoom Out"
                        aria-label="Zoom Out"
                    >
            ...
            <div class="flex items-center gap-2 text-xs text-(--text-secondary) select-none">
                <button
                    @click="emit('prev')"
                    :disabled="!canPrev"
                    class="flex items-center justify-center w-6 h-6 rounded-full disabled:opacity-20 hover:text-(--text-primary)"
                >
    ...
    >
        <button
            @click="emit('show')"
            class="flex items-center justify-center w-8 h-8 rounded-full bg-(--bg-card) border border-(--border-color) shadow-sm text-(--text-tertiary) hover:text-(--text-primary) transition-colors focus-ring-minimal"
            title="Show Controls"
